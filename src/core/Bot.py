...
__all__ = ("Punisher", "bot")


on_startup: List[Callable[["Punisher"], Coroutine]] = []


class Punisher(commands.AutoShardedBot):
    ...
    def reboot(self):
        return os.system("pm2 reload Punisher")
    ...
    async def on_ready(self):
        print(f"[Punisher] Logged in as {self.user.name}({self.user.id})")
