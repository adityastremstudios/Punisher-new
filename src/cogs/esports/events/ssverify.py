...
from core import Cog, Context, PunisherRatelimiter
...
class MemberLimits(defaultdict):
    def __missing__(self, key):
        r = self[key] = PunisherRatelimiter(1, 7)
        return r


class GuildLimits(defaultdict):
    def __missing__(self, key):
        r = self[key] = PunisherRatelimiter(10, 60)
        return r
...
class Ssverification(Cog):
    def __init__(self, bot: Punisher):
        ...
        self.__mratelimiter = MemberLimits(PunisherRatelimiter)  # ss/15s by member
        self.__gratelimiter = GuildLimits(PunisherRatelimiter)  # ss/minute by guild
