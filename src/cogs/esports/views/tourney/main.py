...
class TourneyManager(EsportsBaseView):
    ...
    async def initial_embed(self) -> discord.Embed:
        ...
        _e = discord.Embed(
            color=self.bot.color, title="Punisher Smart Tournament Manager", url=self.bot.config.SERVER_LINK
        )
        ...
        _e.set_footer(
            text="Punisher allows unlimited tournaments.",
        ...
        if not await self.ctx.is_premium_guild():
            if await Tourney.filter(guild_id=self.ctx.guild.id).count() >= 1:
                return await self.ctx.error(
                    f"You need Punisher Premium to create more than 1 tournament.\n\n",
                    7,
                )
    ...
    async def download_excel_data(self, interaction: discord.Interaction, button: discord.Button):
        ...
        # if not await self.ctx.is_premium_guild():
        #     return await self.ctx.error(
        #         "You need Punisher Premium to download Ms Excel file containing all the "
