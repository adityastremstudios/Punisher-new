...
class PremiumCog(Cog, name="Premium"):
    def __init__(self, bot: Punisher):
        self.bot = bot
    ...
    async def pstatus(self, ctx: Context):
        """Get your Punisher Premium status and the current server's."""
        ...
        embed = self.bot.embed(ctx, title="Punisher Premium", url=f"{self.bot.config.WEBSITE}")
    ...
    async def prefresh(self, ctx: Context):
        ...
        embed = self.bot.embed(ctx, title="Punisher Scrims Premium Refreshed", url=f"{self.bot.config.WEBSITE}")
        embed.add_field(name="Server", value=btext, inline=False)
        embed.set_thumbnail(url=ctx.guild.me.display_avatar.url)
    ...
    async def premium(self, ctx: Context):
        """Checkout Punisher Premium Plans."""
        _e = discord.Embed(color=0x00FFB3)
        _e.title = "💎 Welcome to Punisher Scrims!"
        _e.description = (
            "__**Perks you get with Punisher Premium**__\n\n"
            f"{emote.white_arrow} Access to `Punisher Premium` bot.\n"
        ...
        _e.set_footer(text="Thank you for being part of the Punisher")
    ...
    async def on_guild_premium_timer_complete(self, timer: Timer):
        ...
        if (_ch := _g.private_ch) and _ch.permissions_for(_ch.guild.me).embed_links:
            _e = discord.Embed(
                color=discord.Color.red(), title="⚠️__**Punisher Premium Subscription Ended**__⚠️", url=config.SERVER_LINK
            )
            _e.description = (
                "This is to inform you that your subscription of Punisher Premium has been ended.\n\n"
    ...
    async def on_premium_purchase(self, txnId: str):
        ...
        with suppress(discord.HTTPException, AttributeError):
            _e = discord.Embed(
                color=discord.Color.gold(), description=f"Thanks **{member}** for purchasing Punisher Premium."
            )
        ...
        _e = discord.Embed(
            ...
            title="Punisher Premium Purchase Successful!",
            url=self.bot.config.SERVER_LINK,
            description=(
                f"{random_greeting()} {member.mention},\n"
                f"Thanks for purchasing Punisher Premium. Your server **{upgraded_guild}** has access to Punisher Premium features until `{_guild.premium_end_time.strftime('%d-%b-%Y %I:%M %p')}`.\n\n"
                "[Click me to Invite Punisher Bot to your server](https://discord.com/oauth2/authorize?client_id=902856923311919104&scope=applications.commands%20bot&permissions=21175985838)\n"
            ),
        )

        if member not in self.bot.server.members:
            _e.description += f"\n\n[To claim your Premium Role, DM 374122997235187713 on discord]({self.bot.config.SERVER_LINK})."
...
async def setup(bot: Punisher) -> None:
