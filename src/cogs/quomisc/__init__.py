...
from core import Cog, Context, PunisherView
...
class Quomisc(Cog, name="quomisc"):
    ...
    async def source(self, ctx: Context, *, search: typing.Optional[str]):
        ...
        if search is None:
            e = Embed(
                ...
                description=(
                    f"**Original Punisher Source:** [Click Here]({og_source_url})\n"
                ),
                color=config.COLOR
            )
            e.set_footer(text="Punisher Esports")
    ...
    async def invite(self, ctx: Context):
        """Punisher Invite Links."""
        v = discord.ui.View(timeout=None)
        v.add_item(
            discord.ui.Button(
                style=discord.ButtonStyle.link, label="Invite Punisher (Me)", url=self.bot.config.BOT_INVITE, row=1
            )
        )
        v.add_item(
            discord.ui.Button(
                style=discord.ButtonStyle.link, label="Invite Punisher", url=self.bot.config.PRO_LINK, row=2
            )
        )
        v.add_item(
            discord.ui.Button(
    ...
    async def make_private_channel(self, ctx: Context) -> discord.TextChannel:
        ...
        channel = await guild.create_text_channel(
            "punisher-private", overwrites=overwrites, reason=f"Made by {str(ctx.author)}"
        )
        ...
        e.add_field(
            ...
            inline=False,
            value="This channel is made for Punisher to send important announcements and activities that need your attention. If anything goes wrong with any of my functionality I will notify you here. Important announcements from the developer will be sent directly here too.\n\nYou can test my commands in this channel if you like. Kindly don't delete it , some of my commands won't work without this channel.",
        )
        e.add_field(
    ...
    async def setup_cmd(self, ctx: Context):
        """
        Setup Punisher in the current server.
        This creates a private channel in the server. You can rename that if you like.
        Punisher requires manage channels and manage wehooks permissions for this to work.
        You must have manage server permission.
        """

        _view = SetupButtonView(ctx)
        _view.add_item(PunisherView.tricky_invite_button())
    ...
    def format_commit(commit):  # source: R danny
        ...
        return f"[`{short_sha2}`]() {truncate_string(short,40)} ({offset})"
    ...
    async def about(self, ctx: Context):
        ...
        embed.title = f"{emote.bot} Quotient"
    ...
    async def color(self, ctx: Context, *, new_color: QuoColor):
        """Change color of Punisher's embeds"""
    ...
    async def footer(self, ctx: Context, *, new_footer: str):
        """Change footer of embeds sent by Punisher"""
    ...
    async def vote(self, ctx: Context):
        e = self.bot.embed(ctx, title="Vote for Punisher")
        e.description = (
            "**Rewards**\n"
